...
if 'Water Level' in df.columns:
    ...
    fig = px.histogram(
        ...
        nbins=30,
        marginal="box",
        histnorm='probability',  # 👈 normalize counts
        title="Distribution of Cleaned Water Level"
    )
    fig.update_yaxes(range=[0, 2])  # 👈 scale the y-axis between 0 and 2
    st.plotly_chart(fig, use_container_width=True)

    if show_explanations:
        st.markdown("""
        **Explanation:**  
        This histogram shows the normalized distribution of `Water Level` after cleaning non-numeric characters
